  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed inset-0 z-50 bg-[#1a1a1a]/40 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto transition-opacity duration-300 ease-in-out ${
          checkoutOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
      >
        {/* Modal Glass Panel */}
        <div
          onClick={(e) => e.stopPropagation()}
          className={`relative w-full max-w-2xl rounded-2xl border border-[#e5e5e5] bg-[#fcfcfc] p-6 shadow-2xl md:p-8 transition-all duration-300 ease-in-out ${
            checkoutOpen ? "scale-100 opacity-100 translate-y-0 pointer-events-auto" : "scale-95 opacity-0 translate-y-4 pointer-events-none"
          }`}
        >
